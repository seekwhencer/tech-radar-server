        ...
        return new Promise((resolve, reject) => {
            ...
            APIAPP.use(new RegExp(`${rootEndpoint}`), (req, res, next) => {
                ...
                if (this.options.root_endpoint) {
                    ...
                    if (this.options.root_endpoint !== urlSplit[1]) {
                        return next();
                    }
                    if (['login', 'internal', 'status'].includes(urlSplit[2])) {
                        return next();
                    }

                    // the radar index
                    if(['radar'].includes(urlSplit[2]) && !urlSplit[3]){
                        return next();
                    }
                    ...
                    if (['', 'login', 'status', 'internal', 'js', 'css', 'images'].includes(req.originalUrl.split('/')[1])) {
                        return next();
                    }
                }
